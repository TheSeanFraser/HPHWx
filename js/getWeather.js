...
function applyTWindSpeed(data) {
	var timezoneAdjustment =  Math.abs(data.timezone_offset) / 3600;
	for (let i = startHour; i < startHour + 10; i++) {
		if (data.hourly[i + timezoneAdjustment] !== undefined){
...
function applyYWindSpeed(data){
	...
	for (let i = startHour; i < startHour + 10; i++) {
		var tableRowIndex = i - startHour;
		var curWindSpeed = data.hourly[i + timezoneAdjustment].wind_speed;
		var BeaufortWindSpeed = 0;
...
function applyYWindDirection(data){
	...
	for (let i = startHour; i < startHour + 10; i++) {
		var tableRowIndex = i - startHour;
		var curWindDeg = data.hourly[i + timezoneAdjustment].wind_deg;
		var WindDirStr = "NW";
...
function applyYTemp(data){
	...
	for (let i = startHour; i < startHour + 10; i++) {
		var tableRowIndex = i - startHour;
		var curTemp = data.hourly[i + timezoneAdjustment].temp;
		var CelciusTemp = Math.floor(data.hourly[i].temp - 273.15);
...
function applyYHumidity(data){
	...
	for (let i = startHour; i < startHour + 10; i++) {
		var tableRowIndex = i - startHour;
		var curHumidity = data.hourly[i + timezoneAdjustment].humidity;
...
function applyYPressure(data){
	...
	for (let i = startHour; i < startHour + 10; i++) {
		var tableRowIndex = i - startHour;
		var curPressure = data.hourly[i + timezoneAdjustment].pressure;
...
function applyYCloudCover(data){
	...
	for (let i = startHour; i < startHour + 10; i++) {
		var tableRowIndex = i - startHour;
		var curClouds = data.hourly[i + timezoneAdjustment].clouds;
...
function applyYVisibility(data){
	...
	for (let i = startHour; i < startHour + 10; i++) {
		var tableRowIndex = i - startHour;
		var curVisibility = data.hourly[i + timezoneAdjustment].visibility;
		var convertedVisibility = curVisibility / 1000;
...
function applyYPrecipitation(data){
	...
	for (let i = startHour; i < startHour + 10; i++) {
		var tableRowIndex = i - startHour;
		var curPrec = data.hourly[i + timezoneAdjustment].weather[0].main;
		var convertedPrec = 0;
